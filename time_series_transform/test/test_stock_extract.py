...
class Test_stock_extractor:
    ...
    def test_yahoo_extractor_intraday_data(self):
        symbol = 'aapl'
        dat = [date.today() - timedelta(days=1),date.today()]

        if periodBusinessday('2d') == 0:
            pass
        else:
            se = Stock_Extractor(symbol, 'yahoo')
            data = se.get_intra_day(dat[0].strftime('%Y-%m-%d'),dat[1].strftime('%Y-%m-%d'), '1m')
